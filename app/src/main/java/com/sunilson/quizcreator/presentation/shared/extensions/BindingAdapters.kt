package com.sunilson.quizcreator.presentation.shared.extensions
...
fun setTextValueListener(
    editTextWithVoiceInput: EditTextWithVoiceInput,
...
fun LinearLayout.setArchiveAnswers(question: Question?) {
    question?.answers?.forEachIndexed { index, answer ->
        val binding = DataBindingUtil.inflate<ArchiveQuestionAnswerBinding>(
            LayoutInflater.from(context),
            R.layout.archive_question_answer,
            this,
            true
        )
    ...
    if (correct != null) {
        ...
        if (correct) {
            ...
            ImageViewCompat.setImageTintList(
                this,
                ColorStateList.valueOf(ContextCompat.getColor(context, R.color.correct))
            )
        } else {
            this.setImageDrawable(
                androidx.core.content.ContextCompat.getDrawable(
            ...
            ImageViewCompat.setImageTintList(
                this,
                ColorStateList.valueOf(ContextCompat.getColor(context, R.color.wrong))
            )
        }
        ...
        if (marked) {
            ...
            ImageViewCompat.setImageTintList(
                this,
                ColorStateList.valueOf(ContextCompat.getColor(context, R.color.unselected))
            )
        } else {
                ...
                if (vis == View.VISIBLE) {
                    if (this@setInitialAnswers.childCount == 0) {
                        TransitionManager.beginDelayedTransition(
                            this@setInitialAnswers.rootView as ViewGroup,
                            Fade()
                        )
                        answers.forEachIndexed { index, answer ->
                            val answerView = EditTextWithVoiceInput(
                                context,
                                index > OPTIONAL_THRESHOLD,
                                (type != null && type == QuestionType.MULTIPLE_CHOICE),
                                answer = answer
                            )
    ...
    if (answer != null) {
        ...
        if (toggable) {
            if (answer.correctAnswer) {
                ImageViewCompat.setImageTintList(
                    view,
                    ColorStateList.valueOf(ContextCompat.getColor(view.context, R.color.correct))
                )
            } else {
                ImageViewCompat.setImageTintList(
                    view,
                    ColorStateList.valueOf(ContextCompat.getColor(view.context, R.color.unselected))
                )
            }
            ...
            if (answer.correctAnswer) {
                ...
                ImageViewCompat.setImageTintList(
                    view,
                    ColorStateList.valueOf(ContextCompat.getColor(view.context, R.color.disabled))
                )
            }
        }
    } else {
...
fun setTintCompat(imageView: ImageView, @ColorInt color: Int) {
        imageView.imageTintList = Converters.convertColorToColorStateList(color)
}
...
fun BarChart.sevenDayQuizAmount(days: List<Int>, goodDays: List<Int>) {
    ...
    val entries =
        days.reversed().mapIndexed { index, value -> BarEntry(index.toFloat(), value.toFloat()) }
    val goodEntries = goodDays.reversed()
        .mapIndexed { index, value -> BarEntry(index.toFloat(), value.toFloat()) }
    ...
    this.xAxis.valueFormatter = object : ValueFormatter() {
        override fun getAxisLabel(value: Float, axis: AxisBase?): String {
            return when (dataLabels[value.toInt()]) {
...
fun LinearLayout.categorySuccessRates(
