...
class MultipleChoiceQuestionCardView(
    context: Context,
    question: Question,
    questionSubmitted: (Boolean) -> Unit
) : QuestionCardView(context, question, questionSubmitted) {

    val binding: QuizQestionCardViewMultipleBinding =
        QuizQestionCardViewMultipleBinding.inflate(inflater, this, true)
    ...
    init {
        ...
        confirm_question_button.setOnClickListener {
            ...
            val containsIncorrectAnswers =
                question.answers.any { (!it.correctAnswer && it.chosen) || (it.correctAnswer && !it.chosen) }
            if (containsIncorrectAnswers) this.startAnimation(
                android.view.animation.AnimationUtils.loadAnimation(
                    context,
                    R.anim.error_card_animation
                )
            )
