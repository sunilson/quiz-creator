...
@FragmentScope
class AllQuestionsViewModel @Inject constructor(val application: Application, repository: IQuizRepository, eventBus: EventBus) : BaseViewModel(repository) {
    ...
    fun loadQuestions() {
        loading.set(true)
        disposable.add(repository.loadQuestionsOnce(selectedSearchCategory.get()?.id, searchQuery.get()).subscribe({
