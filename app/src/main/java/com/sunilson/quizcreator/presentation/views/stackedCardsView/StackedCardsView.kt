...
class StackedCardsView(context: Context, attrs: AttributeSet) : RelativeLayout(context, attrs) {
    ...
    init {
        ...
        maxChildHeight =
            a.getInt(R.styleable.StackedCardsView_maxChildHeight, 450.convertToPx(context))
        maxChildSpacing =
            a.getInt(R.styleable.StackedCardsView_topAndBottomSpacing, 50.convertToPx(context))
        ...
        addAnimationType =
            StackAnimationTypes.values()[a.getInt(R.styleable.StackedCardsView_addAnimationType, 0)]
        a.recycle()
    }
    ...
    fun removeCard(animationTypes: StackAnimationTypes, finished: (() -> Unit)? = null) {
        ...
        if (childCount != 0) {
            ...
            when (animationTypes) {
                StackAnimationTypes.TRANSLATE_DOWN -> {
                    val animationDistance =
                        height - (currentPopTarget?.layoutParams as RelativeLayout.LayoutParams).topMargin
                    currentPopAnimation =
                        currentPopAnimation!!.translationY(animationDistance.toFloat())
                }
                StackAnimationTypes.TRANSLATE_UP -> {
                    val animationDistance =
                        height - (currentPopTarget?.layoutParams as RelativeLayout.LayoutParams).topMargin
